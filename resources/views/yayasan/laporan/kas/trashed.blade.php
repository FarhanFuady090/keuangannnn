<x-layout-yayasan>
    ...
    <div class="container mx-auto mt-10 px-4">
                    ...
                    @forelse ($trashedTransaksiKas as $transaksi)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 space-x-2">
                                <form action="{{ route('yayasan.laporan.kas.restore', $transaksi->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-blue-600 hover:text-blue-800 font-medium">Restore</button>
                                </form>

                                {{--
                                <form action="{{ route('yayasan.laporan.kas.forceDelete', $transaksi->id) }}" method="POST" onsubmit="return confirm('Hapus permanen?')" class="inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="mt-6">
            <a href="{{ route('yayasan.laporan.kas.index') }}" class="inline-block text-blue-600 hover:underline text-sm">
